...
class MosaicHttp extends Http_1.Http {
    ...
    getMosaicsFromNamespace(namespaceId, queryParams) {
        return this.getNetworkTypeObservable().pipe(operators_1.mergeMap((networkType) => rxjs_1.from(this.mosaicRoutesApi.getMosaicsFromNamespace(namespaceId.toHex(), queryParams != null ? queryParams : {})).pipe(operators_1.map((mosaicsDefinitionDTO) => {
    ...
    getMosaicsName(mosaicIds) {
        ...
        return rxjs_1.from(this.mosaicRoutesApi.getMosaicsName(mosaicIdsBody)).pipe(operators_1.map((mosaicInfosDTO) => {
            return mosaicInfosDTO.map((mosaicInfoDTO) => {
